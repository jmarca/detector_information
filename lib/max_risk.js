...
function requestGenerator(_uri,cuser,cpass,debug){
    return function createRequestObject(){
        ...
        // there is no need for auth here...just a view.
...
function db_view ( model, view ){
    ...
    if(model == 'anyaccident'){
        ...
        var view = ['_design',model,'_view',view].join('/');
        url = [db,view].join('/')
    }
    return url;
}
...
function yearly_risk_models(options,cb_){
    ...
    return function yearly_risk_models(req,res,next){
        ...
        var startkey=[];
        var endkey=[];
        if(activeParams.indexOf('year') != -1){
        ...
        if(activeParams.indexOf('vdsid') != -1){
            view = 'id_day';
            var vdsid = match_vdsid(req.params.vdsid);
            startkey.unshift(vdsid)
            endkey.unshift(vdsid)
        }else{
            view = 'fwy_pm_day';
            ...
            if(activeParams.indexOf('pmstart')){
                startkey.unshift(fwy,dir,+req.params.pmstart)
                endkey.unshift(fwy,dir,+req.params.pmend);
            }else{
                startkey.unshift(fwy,dir,0);
                endkey.unshift(fwy,dir,'Z');
            }
        ...
        var url = db_view(req.params.model ? req.params.model : 'anyaccident', view);
