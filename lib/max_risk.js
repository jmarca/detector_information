...
function max_risk(options,cb_){
    ...
    return function max_risk(req,res,next){
        ...
        var startkey = [vdsid,CDBDateString(start,true)].join(' ');
        if(!vdsid) startkey= [1200000,CDBDateString(start,true)].join(' ');
        // while testing, get one day
        start.setDate( start.getDate() +1 )
        var endkey = [vdsid,CDBDateString(start ,true)].join(' ');
        if(!vdsid) endkey= [1300000,CDBDateString(start,true)].join(' ');

        // use request library to query couchdb.
