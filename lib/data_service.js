...
function vds_data_service(options,cb_){
    ...
    return function vds_data_service(req,res,next){
        ...
        console.log('handling request for vds_data_service');
        ...
        // var data_handler = streamData(res,cb__);
        ...
        _.each(argslist, function(year){
	    // use request library instead of couchdb.js to try to stream output
            var dbname = ['vdsdata',district,year,req.params.vdsid].join('%2f');
	    var x = createRequestObject([dbname,'_all_docs'].join('/')
					,'GET'
					,{'startkey':startkey
					  ,'endkey':endkey
					  ,'include_docs':true}
...
function streamData(res,next){
    var columns = [
        "nl1" ,"ol1" ,"nr3" ,"or3" ,"nr2" ,"or2" ,"nr1" ,"or1" ,"mean.vol.1"
       ,"mean.vol.m" ,"mean.vol.r" ,"sd.vol.1" ,"sd.vol.m" ,"sd.vol.r"
,"cv.occ.1" ,"cv.occ.m" ,"cv.occ.r" ,"cv.volocc.1" ,"cv.volocc.m"
,"cv.volocc.r" ,"cor.vol.1.m" ,"cor.vol.1.r" ,"cor.vol.m.r"
,"cor.occ.1.m" ,"cor.occ.1.r" ,"cor.occ.m.r" ,"cor.volocc.1.m"
,"cor.volocc.1.r" ,"cor.volocc.m.r" ,"autocor.vol.1" ,"autocor.vol.m"
,"autocor.vol.r" ,"autocor.occ.1" ,"autocor.occ.m" ,"autocor.occ.r"
,"ts"
    ];
    res.write('{"columns":'+JSON.stringify(columns)+',"data":[');
    var second_comma = function(r){res.write(','+JSON.stringify(r));};
    function writer(r){
    ...
    return function(err,docsarray){
        ...
        _.each(docsarray,function(docs){
            ...
            _.each(docs.rows,function(row){
                ...
                for(var data_idx = 0; data_idx < doc.data.ts.length; data_idx++){
                    var nullrow = true;
                    var record = columns.map(function(col){
                        var val;
                        if(doc.data[col]){
                            if(nullrow && col!=='ts') nullrow = false;
                            val = doc.data[col][data_idx];
                        }
                        return val;
                    });
                    if(!nullrow){
...
function trimOneHourData(hour,next){
    ...
    return function(err,docs){
	    ...
	    // slide forward to the end of the day!  oh yeah this is daily documents!
...
// nfc bug fix
                        if(result[key].length != idx && val.length){
                            result[key][idx]=val[0];
                            result[key].push(val.slice(1));
                        }else{
                            result[key].push(val);
