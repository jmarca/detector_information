...
var vdsDayDocs = function(dayId,dyv_db,cb){
    return function(err,r){
        ...
        if(!r.rows.length){return cb('no data',dayId);}
                   ...
                   ,function(err,r){
                       ...
                       if(bulkdocs.length){
                           con.bulkDocs({'docs':bulkdocs},function(err,r){
                               ...
                               return    outer_cb(null,dayId);

                               //throw new Error();
                           });
                       }else{
                           return outer_cb("no bulk docs",dayId);
                       }
                       return 1;
                   });
        return true;
...
var vdsFixer = function( vds_id, district, year, cb ){
    ...
    var cb2 = function(err,other){
        ...
        console.log('outer callback ' + doneCount + ':' + queries.length);
