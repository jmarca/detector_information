...
var vdsFixer = function( vds_id, district, year, cb ){
    ...
    var cb2 = function(err,other){
	    ...
	    clientRemote.replicate( [replicate_root,dyv_db].join('/')
				    ,['vdsdata',district,year,vds_id].join('/')
				    ,function(e,r){
					if(e) console.log(JSON.stringify(e));
					console.log('replication for '+dyv_db);
					return 1;
				    });
...
var iterationer = function(err,vds_id){
	...
	remote_compact(vds_id,function(e,b){
	    console.log('done with compaction calls');
	});
    }
    if(vdslist.length){
        vdsLoop();
