...
function wim_twim_copy(options){
    ...
    var query = 'select site_no,loc,wim_type,cal_pm,cal_pm_numeric,latitude,longitude,last_modified,freeway_id as freeway,direction,st_asGeoJSON(geom,7,2) as geojson from '+table;
    ...
    function get_handler(properties,cb){
        ...
        return function(e,r,b){
            ...
            doc.properties[properties.last_modified]={};
            _.each(properties
                  ,function(val,key){
                       if(key === 'geojson'){
                           doc.properties[properties.last_modified][key]=JSON.parse(val);
                       }else if(['site_no', 'direction','last_modified'].indexOf(key) == -1){
                           doc.properties[properties.last_modified][key]=val;
                       }
                   });
    ...
    function handleRow(row){
        ...
        return request.get(url,get_handler(row,function(e){if(e) throw new Error(e);}))
    }
